...
### 2026-09-06 — Session 20: 3 primary skills per project, +N counter pill, equal tile heights & AI dash cleanup

- **`src/data/projectsData.ts`:**
  - Prioritized the exact 3 skills requested for each of the 8 projects:
    1. Otter Body Robot: `Bio-Inspired Robotics`, `Mechatronic System Design`, `Control Systems`.
    2. Micromouse Robot Argo: `Robotics`, `SLAM`, `Embedded Navigation`.
    3. DIYAKAWA 3.0: `Underwater Robotics`, `Electronic Circuit Design`, `Waterproofing`.
    4. Bicycle & Rider Data System: `Instrumentation Systems`, `Kalman Filter`, `Embedded Systems`.
    5. Computer Vision SCARA Robot: `Robotics`, `Computer Vision`, `Embedded Systems`.
    6. Factory Floor Optimization: `Production Flow Analysis`, `Process Optimization`, `Tecnomatix`.
    7. Reverse Engineering of IR Cooker: `Reverse Engineering`, `CAD Modelling`, `Product Analysis`.
    8. Vintage Movie Projector: `Manufacturing Technologies`, `Design for Manufacturing`, `Product Development`.
  - Replaced all AI em-dashes (`—`) in project titles, short descriptions, headings, and captions with natural punctuation (periods, commas, colons).
- **`src/components/Projects.tsx` (iteration 4):**
  - Updated card renderer to slice the top 3 tags and display a `+N` badge (e.g. `+5`, `+10`) for the remaining skills.
  - Replaced em-dash in section subtitle with comma.
- **`src/components/Hero.tsx`:**
  - Replaced em-dash in subtitle with comma.
- **`src/index.css`:**
  - Styled `.project-tile-tag-more` counter badge with distinct emphasis and hover accent glow.
  - Enforced uniform card height across the grid: fixed `min-height: 3.3rem` on `.project-tile-title` to align 1-line and 2-line titles, `min-height: 2.2rem` on `.project-tile-tags`, `flex: 1 1 auto; min-height: 4.8rem;` on `.project-tile-desc`, and `margin-top: auto;` on `.project-tile-footer`.
- **Build verified:** `bun tsc --noEmit` passes with 0 errors; `bun run build` OK (67 modules).

## Changelog / Build Log

### 2026-09-06 — Session 19: Full interactive project cards, hover highlights, descender fix & Intro space cleanup

- **`src/components/Projects.tsx` (iteration 3):**
  - Replaced expandable `+` tile model with full interactive project cards shown by default (matching reference pattern on `jrdan.dev` with custom enhancements).
  - Each card renders: cover image with subtle hover zoom (`1.05x`), project title, skills & technologies tag pills (`project.tags`), compact intro description (`project.shortDesc`), and "View Project" footer with arrow icon.
  - Entire card is clickable (`onClick={() => onViewDetails(project.id)}`, `role="button"`, `tabIndex={0}`, keyboard navigation on Enter/Space).
- **`src/index.css`:**
  - **Hover Effects on Cards:** added card lift (`transform: translateY(-6px)`), radiant gradient glow backdrop derived from `--tile-gradient`, accent border highlight (`--project-accent`), title text lighting up in accent color, and "View Project" arrow shifting right (`translateX(4px)`).
  - **Skills Tags:** styled tag pills with translucent background, border, and clean typography (`0.74rem`, `font-weight: 600`).
  - **Title Descender Cutoff Fix:** added `display: inline-block; line-height: 1.25; padding-bottom: 0.15em;` on `.projects-title` so descenders on "y" and "j" are never clipped by the gradient text clipping mask or subsequent content.
  - **Intro Section Spacing Cleanup:** removed `min-height: 180vh;` from `.intro` and set `padding: 12vh 4rem 12vh;` so the section naturally concludes after the bio content, eliminating the huge empty white void between About Me and Projects.
- **Build verified:** `bun tsc --noEmit` passes with 0 errors; `bun run build` OK (67 modules).
